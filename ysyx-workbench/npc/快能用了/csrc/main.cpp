...
//打开外部程序到pmem_rom
  if(argv[1] == NULL){
    load_bin_to_rom("/home/shuyv/ysyx-workbench/am-kernels/tests/cpu-tests/build/bit-riscv32e-npc.bin");
  }
  else{
...
// 将测试指令复制到 pmem 起始位置（CPU 从 0x80000000 地址取指）
  ...
  //将 std::vector pmem 的数据指针赋值给 pmem_ptr
  pmem_ptr = pmem.data();
  ...
  while(1) {
    ...
    // }
    // uint32_t read_addr = 0x80008fe4;
    // uint32_t read_data = pmem_read(read_addr);
    // if(count < 100)printf("【从 0x%08x 读取到数据：0x%08x\n", read_addr, read_data);
