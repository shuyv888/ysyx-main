...
void pmem_write(int waddr, int wdata, my_byte wmask) {
    ...
    if (addr >= PMEM_BASE && addr < (PMEM_BASE + (MEM_SIZE << 2))) {
        ...
        if (wmask == 0xf) {
            ...
            // sb / sh 部分写（按字节合并）
            pmem[idx] = (wmask == 0x1) ? ((pmem[idx] & 0xFFFFFF00) | (uint32_t)(wdata & 0x000000FF)) :
                        (wmask == 0x2) ? ((pmem[idx] & 0xFFFF00FF) | ((uint32_t)(wdata & 0x0000FF00))) :
                        // sh指令，也许之后会用,可能不全，记得补充
                        //(wmask == 0x3) ? ((pmem[idx] & 0xFFFF0000) | (wdata & 0XFF)) :
                        (wmask == 0x4) ? ((pmem[idx] & 0xFF00FFFF) | ((uint32_t)(wdata & 0x00FF0000))) :
                        (wmask == 0x8) ? ((pmem[idx] & 0x00FFFFFF) | ((uint32_t)(wdata & 0xFF000000))) :
                        pmem[idx];
...
// 全局保存最新寄存器状态
uint32_t guest_gprs[32];
uint32_t guest_pc;

// DPI 函数实现
extern "C" void dpi_get_gprs(
    uint32_t gprs[32],   // input array
    uint32_t pc
) {
    // 一口气拷贝 32 个寄存器
    printf("[DPI] cycle update: pc=0x%08x, x0=%08x, a0(x10)=%08x\n", pc, gprs[0], gprs[10]);
    memcpy(guest_gprs, gprs, sizeof(guest_gprs));
    guest_pc = pc;
    printf("[DPI] dpi_get_gprs called, pc=0x%08x\n", guest_pc);
}
